        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-6">
                <div>
                    ...
                    <p>Current Price: ${{ number_format($car->bids()->max('amount') ?? $car->start_price) }}</p>
                    <p>Bidding Ends: {{ optional($car->bid_end_time)->toDayDateTimeString() }}</p>
                </div>
                <div class="mt-4">
                    <form action="{{ route('bidding.bid', ['car' => $car->id]) }}" method="post">
                        @csrf
                        <input type="number" name="amount" placeholder="Enter your bid" min="{{ $car->bids()->max('amount') ? $car->bids()->max('amount') + 1 : $car->start_price }}" required class="border-gray-300 rounded-md shadow-sm mt-1 block w-full">
                        <x-button class="mt-3">
                            Place Bid
                        </x-button>
                    </form>
